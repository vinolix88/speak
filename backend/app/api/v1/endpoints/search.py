...
async def search_chats(
    ...
    for chat in chats:
        if chat.type == 'group' and chat.name and q.lower() in chat.name.lower():
            ...
            # Находим собеседника
            part = await db.execute(select(ChatParticipant.user_id).where(ChatParticipant.chat_id == chat.id))
            ...
            if other_id:
                ...
                if other_user and q.lower() in other_user.username.lower():
                    result.append(chat)
    # Используем функцию формирования ответа из chats.py (она должна быть экспортирована)
    from app.api.v1.endpoints.chats import _build_chat_response
    responses = []
    for chat in result:
